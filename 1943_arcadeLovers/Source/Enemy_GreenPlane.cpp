...
Enemy_GreenPlane::Enemy_GreenPlane(int x, int y) : Enemy(x, y)
{
	...
	*/
	...
	path.PushBack({ 0.0f, 1.0f }, 450, &flyAnim);
...
void Enemy_GreenPlane::Update()
{
	...
	{
		coolDown = 0;
		App->particles->AddParticle(App->particles->enemyShoot, position.x + 5, position.y + 5, Collider::Type::ENEMY_SHOT, true);
		App->particles->AddParticle(App->particles->enemyShootL, position.x + 5, position.y + 5, Collider::Type::ENEMY_SHOT, true);
		App->particles->AddParticle(App->particles->enemyShootR, position.x + 5, position.y + 5, Collider::Type::ENEMY_SHOT, true);
	}
