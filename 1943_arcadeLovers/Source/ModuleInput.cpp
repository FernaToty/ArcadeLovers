...
UpdateResult ModuleInput::PreUpdate()
{
	...
	if (App->input->keys[SDL_SCANCODE_ESCAPE] == KeyState::KEY_DOWN) return UpdateResult::UPDATE_STOP;
...
bool ModuleInput::ShakeController(int id, int duration, float strength)
{
	...
	else
	{
		...
		SDL_HapticRumblePlay(pad.haptic, strength, duration / 60 * 100); //Conversion from frames to ms at 60FPS
